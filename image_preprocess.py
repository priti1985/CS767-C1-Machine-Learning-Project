...
def main(input_db, photo_dir, output_dir, min_score=1.0, img_size=165, split_ratio=0.8):
    ...
    clean_and_resize(db, photo_dir, train_indices, min_score, img_size, train_writer, crop_dir,output_dir)

    clean_and_resize(db, photo_dir, test_indices, min_score, img_size, val_writer, crop_dir,output_dir)


def clean_and_resize(db, photo_dir, indices, min_score, img_size, writer, crop_dir,output_dir):
    ...
    for i in tqdm(indices):
        ...
        img_path = os.path.abspath(os.path.join(photo_dir, filename))
        ...
        img_age = int(age[i])
        if img_age>=0 and img_age<=3:
            img_age = 0
        elif img_age>=4 and img_age<=7:
            img_age = 1
        elif img_age>=8 and img_age<=14:
            img_age = 2
        elif img_age>=15 and img_age<=24:
            img_age = 3
        elif img_age>=25 and img_age<=32:
            img_age = 4
        elif img_age>=33 and img_age<=43:
            img_age = 5
        elif img_age>=44 and img_age<=59:
            img_age = 6
        else:
            img_age = 7
        ...
        if img is not None:
            ...
            print("Unable to load image at path {}".format(img))
...
if __name__ == '__main__':
    ...
    # main(input_db=r'/Users/pritiagrawal/projectData/imdb_crop/imdb.mat', photo_dir=r'/Users/pritiagrawal/projectData/imdb_crop/'
    #      , output_dir=r'/Users/pritiagrawal/projectData/',
    #      min_score=1.0, img_size=224)
